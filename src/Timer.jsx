...
<<<<<<< HEAD
  ...
  return (
    <>
      <input
        name='name'
        className={timerStyles.timerNameInput}
        type='text'
        value={name}
        onChange={changeName}
      ></input>
...
const ButtonControls = ({ idx, elm }) => {
  ...
  const addEvent = () => {
    dispatch(
      MyStore.setAction(
        MyStore.ADD_TIMER,
        MyStore.createTimerObj(MyStore.TYPE_EVENT_OVER)
      )
    );
  };
  const addStopwatch = () => {
  ...
  return (
    <div className={timerStyles.timerControlButtons}>
...
<<<<<<< HEAD
  ...
  return (
    <div className={timerStyles.eventControls}>
...
const TimerItem = ({ idx, elm, selected }) => {
  const dispatch = useDispatch();
  ...
  return (
    <form
      className={
        !selected
          ? timerStyles.timerListItem
          : timerStyles.timerListItem + ' ' + timerStyles.timerItemSel
      }
      onClick={click}
      onSubmit={formSubmit}
    >
    ...
    return timers.map((el, idx) => {
      return (
        <TimerItem
          ...
          idx={idx}
          elm={el}
          selected={currSelElmIdx === idx}
        />
      );
    });
  }, [timers, currSelElmIdx]); // ,  bRender
  return <div className={timerStyles.timerList}>{timersElements}</div>;
};
...
const TimerTabloid = () => {
  ...
  const renderTimersToDisplay = useMemo(() => {
    const res = timers.map((el, idx) => {
      return el.showProgress === MyStore.DISPLAY_STATUS_SHOW ? (
        <TimerTabloidItem key={idx} elm={el} />
      ) : null;
    });
    return res;
  }, [bRender]);
...
const TimerTabloidItem = ({ elm }) => {
  return (
    <div className={timerStyles.timerTabloidItem}>
      {elm.name}: {elm.type !== MyStore.TYPE_STOPWATCH ? 'осталось' : 'прошло'}{' '}
      {elm.dateString}
    </div>
  );
};
